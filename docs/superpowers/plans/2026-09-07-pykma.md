...
# 테스트 이름은 한국어로 쓴다. 무엇이 깨졌는지 실패 출력에서 바로 읽히는 쪽이
# 낫고, pep8-naming은 비ASCII 식별자를 "소문자가 아니다"로 본다.
"tests/**" = ["N802"]
...
`uv init`이 `authors`에 자동으로 넣는 이메일은 **지운다.** PyPI 메타데이터는 공개되고,
주소를 올릴지는 본인이 정할 일이다.

`[tool.hatch.build.targets.wheel]`이 `src/pykma_mcp`를 가리키므로 Task 8 전에도
디렉터리가 있어야 빌드가 된다. Step 3에서 빈 패키지로 미리 만든다.

- [x] **Step 3: 패키지 초기 파일과 스모크 테스트를 쓴다**
...
MCP 패키지도 빈 채로 만들어 둔다 — 위 hatch 설정이 이 경로를 요구한다:

```bash
mkdir -p src/pykma_mcp
printf '"""pykma의 MCP 서버."""\n' > src/pykma_mcp/__init__.py
touch src/pykma_mcp/py.typed
```
...
- [x] **Step 5: `.env.example`과 README 뼈대를 쓴다**

`.gitignore`에 커버리지 산출물을 더한다:
...
- [x] **Step 6: `__init__.py`에서 공개 API를 재수출한다**
...
- [ ] ⛔ (인증키·PyPI 등록 대기) **Step 5: PyPI에 Trusted Publisher를 등록한다**
...
- [ ] ⛔ (인증키·PyPI 등록 대기) **Step 6: 배포하고 확인한다**
...
- [x] **Step 6: README에 MCP 절을 넣고 배포한다**
...
- [x] **Step 7: 배포된 패키지로 MCP를 다시 붙여 확인한다**
